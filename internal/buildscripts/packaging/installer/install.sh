...
default_network_interface="0.0.0.0"
...
Options:
  ...
  --mode <agent|gateway>                Configure the collector service to run in agent or gateway mode.
                                        (default: "agent")
  --network-interface <ip>              network interface the collector receivers listen on.
...
parse_args_and_install() {
  ...
  local network_interface="$default_network_interface"
